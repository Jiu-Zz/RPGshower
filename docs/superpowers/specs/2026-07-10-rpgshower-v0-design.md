...
已实现角色卡、基础字段编辑、档案视图、JSON 文件导入导出、完整 JSON 编辑器和 Zod 校验。本步补齐 `archive`、`extensions` 的数据结构和兼容测试；档案视图目前为只读展示。

当前仅支持 `schemaVersion: "0.1.0"`。旧文件缺少 `archive` 或其中字段时，文本补空字符串、数组补空数组；缺少 `extensions` 时补空对象。已提供字段仍须通过类型校验。其他版本或缺少版本号将拒绝导入。扩展区允许任意 JSON 值，并在导入导出中保留；核心对象未定义的字段会被移除，自定义字段应放入扩展区。

当前使用 `avatarText` 显示文字头像，图片头像尚未实现。运行命令和开发进展见 [README](../../../README.md)。以下章节保留完整 v0 目标。

当前阶段的任务清单和验收标准见 [v0 阶段目标](../../v0-milestone.md)，v1 及以后方向见 [长期产品路线图](../../long-term-roadmap.md)。

资源字段当前允许非负整数或小数，`current` 不得大于 `max`；Schema 和基础表单使用同一约束。
